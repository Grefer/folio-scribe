...
def default_note(date: str, chinese: bool) -> str:
    """Return a skeleton daily note with frontmatter and section headings."""
    if chinese:
        title = f"# {date} 交易日志"
        plan = "## 08:45 港股交易计划"
        review = "## 16:15 港股交易总结"
...
def replace_section(note: str, start_marker: str, end_marker: str | None, content: str) -> str:
    """Replace content between *start_marker* and *end_marker* in *note*."""
...
def set_frontmatter_model(note: str, model: str | None) -> str:
    """Set the frontmatter model field when a model is known."""
    if not model:
        return note
...
def write_daily_note(
    ...
    """Insert or update a section in an Obsidian daily trading note."""
