...
当前实现状态：Python Futu adapter 是只读且偏报价读取的 beta 实现。账户、持仓、
委托、成交、期权链等读取能力仍在路线图中。
...
Current implementation status: the Python Futu adapter is a read-only,
quote-focused beta. Account, position, order, fill, and option-chain reads are
still roadmap work.
